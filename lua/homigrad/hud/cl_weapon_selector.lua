...
function WS.WeaponSelectorDraw( ply )
    ...
    for i = 0, #Weapons do
        ...
        for Id = 0, #slotTbl do
            ...
            if SelectedWep == wep_lerp[1] then
                wep_lerp[2] = LerpFT(0.05,wep_lerp[2],1)
            else
                wep_lerp[1] = SelectedWep
                wep_lerp[2] = 0
            end

            --print(wepId,wep)
            local sizeH = SelectedWep == wep and (scrH *0.12) * wep_lerp[2] or (scrH *0.025)
            local LastSelected = 0
            if slotTbl[wepId-1] and SelectedWep == slotTbl[wepId-1] then
                lastPos = (scrH *0.095) * wep_lerp[2]
            end
            draw.RoundedBox(
                0,
                position,
            ...
            surface.SetDrawColor( 97,11,173, WS.Transparent*( SelectedWep == wep and 200 or 0 )  )
            surface.SetMaterial( gradient_u )
            surface.DrawTexturedRect( position, (scrH * 0.025) * (Ammout) + (scrH * 0.05) + lastPos, sizeX, sizeH )
            if SelectedWep == wep then
                surface.SetDrawColor( 0, 0, 0, WS.Transparent*150 )
            ...
            if SelectedWep == wep and wep.DrawWeaponSelection then
                wep:DrawWeaponSelection(position + 5, (scrH * 0.025) * (Ammout* wep_lerp[2]) + (scrH * 0.055 * wep_lerp[2]) + lastPos, sizeX - 10, sizeH, WS.Transparent*(255* wep_lerp[2]))
            end
        end
