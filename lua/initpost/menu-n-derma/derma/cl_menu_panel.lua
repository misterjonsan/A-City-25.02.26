local PANEL = {}
local curent_panel 
local red_select = Color(138,0,192)

local Selects = {
    {Title = "Отключиться", Func = function(luaMenu) RunConsoleCommand("disconnect") end},
    {Title = "Меню ГМОДа", Func = function(luaMenu) gui.ActivateGameUI() luaMenu:Close() end},
    {Title = "Дискорд", Func = function(luaMenu) luaMenu:Close() gui.OpenURL("https://discord.gg/Dde2bp2G9A")  end},
    {Title = "Роль трейтора",
    GamemodeOnly = true,
    ...
    {Title = "Аппиренс", Func = function(luaMenu,pp) hg.CreateApperanceMenu(pp) end},
...
function PANEL:InitializeMarkup()
    ...
    if hg.PluvTown.Active then
        local text = "<font=ZC_MM_Title><colour=192,2,199>    </colour>City</font>\n<font=ZCity_Tiny><colour=105,105,105>" .. gm .. "</colour></font>"
    ...
    local text = "<font=ZC_MM_Title><colour=192,2,199>A</colour>-City</font>\n<font=ZCity_Tiny><colour=105,105,105>" .. gm .. "</colour></font>"
    return markup.Parse(text)
end
...
function PANEL:Init()
    ...
    function git:DoClick()
        surface.PlaySound("homigrad/vgui/item_showcase_sticker_01.wav")
...
function PANEL:AddSelect( pParent, strTitle, tbl )
    ...
    function btn:Think()
        ...
        local anim = math.Clamp((self.Time - CurTime()) / 0.1,0,1)

        if anim < 0.75 and !self.played then
            self.played = true

            surface.PlaySound("homigrad/vgui/item_sticker_select.wav")
        end

        self:SetAlpha(alpha * (1 - anim))
        local v = self.HoverLerp
        self:SetTextColor(self.RColor:Lerp(red_select, v + anim))
        ...
        if (crw ~= targetText) or (curent_panel == string.lower(strTitle)) then
            local ntxt = ""
            local will_text = (curent_panel == string.lower(strTitle) and not strTitle == 'Роль трейтора') and '[ '..string.upper(strTitle)..' ]' or strTitle
